...
Runtime evidence records an explicit `storage_read_unit`. Bucket-native time-series queries report logical `time_series_bucket` reads from the access-path counter, and scalar range scans report one `relational_index_probe` per query. These logical access-path units keep identical cold and cached invocations invariant; other embedded paths report `runtime_storage_read`. Cache-dependent physical reads must not masquerade as invariant logical work, and setup must not warm the timed query to hide that distinction.
