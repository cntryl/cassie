...
Cassie repair does not undo user data or copy data between nodes. A local snapshot rollback rehearsal is covered for repaired materialized versions: restore the pre-repair snapshot to a new data directory, verify the target version and active query results, and confirm the post-snapshot repair audit is absent. For an operational rollback, restore a local snapshot, replay/rebuild the projection, activate a verified projection version, or move traffic to another independent node through the external router workflow. Record the integrity report id, repair report id, commands run, operator, timestamp, and follow-up verification state in the deployment incident log.
