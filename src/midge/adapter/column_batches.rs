...
impl Midge {
    ...
    pub fn scan_column_batch_projected_rows(
        ...
        limit: Option<usize>,
    ) -> Result<Option<(Vec<Vec<DocumentRef>>, MidgeScanTimings, String)>, CassieError> {
        ...
        for segment in &metadata.segments {
            ...
            let payload: ColumnBatchPayload = serde_json::from_slice(&raw).map_err(|error| {
                CassieError::Parse(format!("invalid column batch payload: {error}"))
            })?;
