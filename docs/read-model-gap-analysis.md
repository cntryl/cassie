...
- **Retrieval:** full-text search, vector search, pgvector-style operators, hybrid scoring, HNSW/IVFFlat metadata and execution paths, and embedding-provider validation exist. Full-text is currently a query-time rebuild from authoritative rows; persisted postings/statistics and publication debt remain the Phase 5 retrieval gap.
...
- `REPAIR PROJECTION` executes safe local row/range hash rebuilds, verified index-sidecar rebuilds, explicit materialized version rebuilds without activation, or active materialized full rebuilds when the latest integrity report and scope contract are repairable, audits the action, and immediately verifies the projection.
- [Projection Repair Runbook](projection-repair-runbook.md) documents plan, execute, verify, audit, rollback/escalate, and scope-boundary procedures.
...
Index, explicit projection-version, and active materialized full-rebuild repair now execute only under their bounded local mutation contracts. Row/range repair is collection-write-gated, and a 1,024-row/four-range manifest is verified, repaired, exported, and reopened after restart. The existing 10k/100k `projection_verify` scenarios remain the read-scaling feedback loop; production capacity thresholds remain open.
...
- `docs/postgres-compatibility.md` now contains a maintained read-model client matrix for tokio-postgres, psql, sqlx, diesel, Prisma, SQLAlchemy, pgAdmin4, and migration-tool workflows.
- `tests/compatibility_matrix.rs` covers tokio-postgres startup, simple and extended query flows, prepared queries, DDL/DML, session-scope/search-path behavior, `ON CONFLICT`, constraints, missing-schema SQLSTATE metadata, recursive CTEs, and syntax-error recovery.
...
- An ignored optional Prisma CLI probe validates `prisma db pull --print` introspection for compatible row-store tables when Prisma is installed and `CASSIE_RUN_PRISMA_COMPAT=1` is set.
...
- Add sqlx/diesel automation, broader Prisma/SQLAlchemy/pgAdmin4 probes, migration-tool depth, and native extension probes only when they can be isolated from default-suite brittleness.
...
- [Feature Support](feature-support.md): projection repair planning, local row/range/index/projection-version/full-rebuild repair, persisted repair audit reports, snapshot rollback rehearsal, and admin-only/no-distributed repair boundaries.
...
- [PostgreSQL Compatibility](postgres-compatibility.md): default deterministic tokio-postgres baseline plus opt-in psql, Prisma CLI, and SQLAlchemy Core probe coverage.
...
- [Projection Repair Runbook](projection-repair-runbook.md): admin-only local plan, execute, verify, audit, rollback/escalate, and scope-boundary procedures for projection repair.
