...
- `DATE`, `TIME`, and `TIMESTAMP` string input is validated and canonicalized by one shared parser used on write, on `CAST`, by the pgwire binary codec, by `time_bucket`, and by retention. An unparseable string is rejected rather than stored as-is. `TIMESTAMP` values with an explicit UTC offset are converted to UTC; offset-free input is treated as already UTC. The canonical stored form is always UTC with a trailing `Z` (for example `2024-01-01T07:00:00Z`), so `ORDER BY`, equality, and `time_bucket` all agree, and plain text comparison sorts chronologically. This validation and canonicalization applies to new and updated values only; rows written before this contract keep whatever string they already had and are not retroactively rewritten.
