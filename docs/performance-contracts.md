...
`document_create_get/10k` is retained as diagnostic mutation evidence rather than an intended release regression gate. Creating documents against the fully indexed 10k fixture triggers non-stationary index maintenance, so shared-runner timing variance is evidence about the mutation environment, not a stable transport regression signal. The Tier 4 HTTP query and vector-search rows remain the trustworthy release gates and run first against the pristine shared fixture; each HTTP query sample executes a fixed batch of 128 requests, preserving one request as the logical operation while lifting the measured sample above transport scheduling noise. The HTTP query uses a score-index seek and returns 20 rows, keeping the transport measurement within the REST request deadline on the 10k fixture. Tier 4 pgwire simple-query samples likewise execute fixed batches of 64 complete queries, avoiding a duration-loop shape whose intentionally serial throughput can be mistaken for capped capacity. The document row runs last so its state changes cannot contaminate either stable gate, while still enforcing request completion, response correctness, runtime evidence, and artifact retention. Tier 4 and Tier 5 time exactly the named create and get requests, while the Tier 6 soak uses an explicit create/get/delete cycle to keep its hour-long fixture bounded.
