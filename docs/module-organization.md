...
- These targets are engineering guardrails enforced through review, focused refactors, and the audit command in `AGENTS.md`; they are not automated layout gates.

## Executor

`src/executor/mod.rs` is the public facade. Keep public exports there and keep execution internals under `src/executor/execution/`. Split by behavior family: dispatch and plan inspection, source production, DML/schema command execution, access-path implementations, scoring, aggregation, windows, and result assembly.
...
Lifecycle, query, document, vector, role, diagnostics, replay, snapshot, transaction, and operational methods stay in focused service modules and extend `Cassie` through focused impl blocks.
