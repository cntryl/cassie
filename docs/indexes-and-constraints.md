...
- Bucket-native membership is persisted in the Midge data family under the lexkey v4 `time-series-index` key family with hierarchical database/schema/relation segments. The stored JSON value carries `collection`, `index_name`, `id`, `bucket_key`, and `timestamp`.
