...
`document_create_get/10k` is retained as diagnostic mutation evidence rather than an intended release regression gate. Creating documents against the fully indexed 10k fixture triggers non-stationary index maintenance, so shared-runner timing variance is evidence about the mutation environment, not a stable transport regression signal. The Tier 4 HTTP query and vector-search rows remain the trustworthy release gates and run first against the pristine shared fixture; each HTTP query sample executes a fixed batch of 128 requests, preserving one request as the logical operation while lifting the measured sample above transport scheduling noise. Tier 4 HTTP, pgwire simple-query, and protocol-comparison owners use the same score-index seek and return 20 rows, keeping the HTTP transport measurement within the REST request deadline on the 10k fixture. Tier 4 pgwire multi-statement samples use two indexed 10-row component queries. Tier 4 pgwire simple-query samples likewise execute fixed batches of 64 complete queries, avoiding a duration-loop shape whose intentionally serial throughput can be mistaken for capped capacity. The document row runs last so its state changes cannot contaminate either stable gate, while still enforcing request completion, response correctness, runtime evidence, and artifact retention. Tier 4 and Tier 5 time exactly the named create and get requests, while the Tier 6 soak uses an explicit create/get/delete cycle to keep its hour-long fixture bounded.
...
Duration-based batches record their actual completed logical-operation count and normalize cumulative storage-read, candidate, result-cache, and fallback counters to one operation. Each Tier 3 time-series representative executes two unchanged partition-and-time-bounded queries per measured closure, declares `query` as its logical unit, and reports the exact 512-row cardinality of one query rather than the summed batch cardinality. The registered 15-minute, 1-hour, and 1-day owners use independent copies of the same deterministic 100k source fixture and create the selected bucket index outside the measurement boundary, preventing cross-width derived-state contamination. This batching changes only measurement shape; its fixture, setup boundary, path and fallback proofs, sampling, thresholds, and trust policy remain unchanged.
