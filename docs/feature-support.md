...
- REST HTTP transport rejects request bodies over 8 MiB, limits HTTP/1 header buffering to 32 KiB,
  ...
  return 403; no permissive CORS headers are emitted. Hashed UI assets are immutable-cacheable,
  HSTS is emitted only on TLS responses, and `/metrics` is intentionally public diagnostics.
