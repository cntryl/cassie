...
const LEGACY_VERSION_V5: &[u8] = b"v5";
const LEGACY_LEXKEY: &[u8] = b"lexkey";
...
const FAMILY_VECTOR_INDEX_STATE: &[u8] = b"\x14";
const FAMILY_NORMALIZED_VECTOR: &[u8] = b"\x13";
const FAMILY_INDEX: &[u8] = b"index";
const FAMILY_FULLTEXT_INDEX: &[u8] = b"\x12";
const FAMILY_SCALAR_INDEX: &[u8] = b"\x11";
const FAMILY_TIME_SERIES_INDEX: &[u8] = b"\x15";
const FAMILY_UNIQUE_RESERVATION: &[u8] = b"unique-reservation";
const FAMILY_COLUMN_BATCH: &[u8] = b"\x17";
...
const FAMILY_OBJECT_ID_COUNTER: &[u8] = b"object-id-counter";
...
pub(super) fn legacy_layout_prefixes() -> Vec<Vec<u8>> {
    [LEGACY_VERSION_V2, LEGACY_VERSION_V5]
        .into_iter()
        .map(|version| {
            let parts = [ROOT, LEGACY_LEXKEY, version];
            let mut encoder =
...
pub(super) fn vector_index_state_key(relation_id: u64, field_id: u32) -> Vec<u8> {
    vector_hot_key(FAMILY_VECTOR_INDEX_STATE, relation_id, field_id, &[])
}

pub(super) fn hnsw_graph_node_key(relation_id: u64, field_id: u32, id: &str) -> Vec<u8> {
    vector_hot_key(
...
pub(super) fn hnsw_graph_node_prefix(relation_id: u64, field_id: u32) -> Vec<u8> {
    vector_hot_prefix(FAMILY_VECTOR_INDEX_STATE, relation_id, field_id, &[b"n"])
}

pub(super) fn hnsw_source_summary_key(relation_id: u64, field_id: u32) -> Vec<u8> {
    vector_hot_key(FAMILY_VECTOR_INDEX_STATE, relation_id, field_id, &[b"f"])
}

pub(super) fn vector_index_state_prefix(relation_id: u64) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    prefix(FAMILY_VECTOR_INDEX_STATE, &[relation.as_slice()])
}
...
pub(super) fn normalized_vector_key(relation_id: u64, field_id: u32, id: &str) -> Vec<u8> {
    vector_hot_key(
...
pub(super) fn normalized_vector_prefix(relation_id: u64, field_id: u32) -> Vec<u8> {
    vector_hot_prefix(FAMILY_NORMALIZED_VECTOR, relation_id, field_id, &[])
}

pub(super) fn normalized_vector_collection_prefix(relation_id: u64) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    prefix(FAMILY_NORMALIZED_VECTOR, &[relation.as_slice()])
}

fn vector_hot_key(family: &[u8], relation_id: u64, field_id: u32, suffix: &[&[u8]]) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    let field = field_id.to_be_bytes();
    let mut components = vec![relation.as_slice(), field.as_slice()];
    components.extend_from_slice(suffix);
    key(family, &components)
}

fn vector_hot_prefix(family: &[u8], relation_id: u64, field_id: u32, suffix: &[&[u8]]) -> Vec<u8> {
    let mut value = vector_hot_key(family, relation_id, field_id, suffix);
...
pub(super) fn row_prefix(relation_id: u64) -> Vec<u8> {
    let encoded = encoded_u64_component(relation_id);
    prefix(FAMILY_ROW, &[encoded.as_slice()])
}

pub(super) fn row_key(relation_id: u64, id: &str) -> Vec<u8> {
    let encoded = encoded_u64_component(relation_id);
    key(FAMILY_ROW, &[encoded.as_slice(), id.as_bytes()])
}
...
pub(super) fn column_store_collection_prefix(relation_id: u64) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    prefix(FAMILY_COLUMN_STORE, &[relation.as_slice()])
}

pub(super) fn column_store_row_prefix(relation_id: u64) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    prefix(FAMILY_COLUMN_STORE, &[relation.as_slice(), b"r"])
}

pub(super) fn column_store_row_key(relation_id: u64, id: &str) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    key(
        FAMILY_COLUMN_STORE,
        &[relation.as_slice(), b"r", id.as_bytes()],
    )
}

pub(super) fn column_store_deleted_key(relation_id: u64, id: &str) -> Vec<u8> {
    let relation = encoded_u64_component(relation_id);
    key(
        FAMILY_COLUMN_STORE,
        &[relation.as_slice(), b"x", id.as_bytes()],
    )
}
...
pub(super) fn column_store_field_key(relation_id: u64, field_id: u32, id: &str) -> Vec<u8> {
    ...
    key(
        FAMILY_COLUMN_STORE,
        &[relation.as_slice(), b"f", field.as_slice(), id.as_bytes()],
    )
}
...
pub(super) fn utf8_first_component_after_prefix(key: &[u8], prefix: &[u8]) -> Option<String> {
    let suffix = key.strip_prefix(prefix)?;
    let component = suffix.split(|byte| *byte == LexKey::SEPARATOR).next()?;
    std::str::from_utf8(component).ok().map(ToString::to_string)
}

pub(super) fn utf8_last_component(key: &[u8]) -> Option<String> {
    key.split(|byte| *byte == LexKey::SEPARATOR)
...
pub(super) fn scalar_index_entry_key(
    ...
    let mut key = scalar_index_data_prefix(relation_id, index_id);
    for value in values {
...
pub(super) fn scalar_index_seek_prefix(
    ...
    let mut seek_prefix = scalar_index_data_prefix(relation_id, index_id);
...
fn encoded_u64_component(value: u64) -> Vec<u8> {
    let mut encoder = Encoder::with_capacity(9);
    encoder.encode_u64_into(value);
    encoder.into_vec()
}
...
fn encode(family: &[u8], components: &[&[u8]], trailing_separator: bool) -> Vec<u8> {
    let mut parts = Vec::with_capacity(3 + components.len());
    parts.push(ROOT);
    parts.push(BASELINE);
    parts.push(family);
