...
Cassie is a single-node-first query engine for read models in CQRS and event-sourced systems. The event stream is the source of truth; Cassie materializes and serves projection data through PostgreSQL wire protocol, with a secondary administrative REST API.
...
Cassie does not pursue distributed SQL, cross-node transactions, consensus, multi-node query planning, OLTP optimization, trigger-based business logic, or a second storage abstraction. Independent nodes scale through external routing and projection ownership.
