...
| --- | --- | --- | --- | --- |
| Confirmed drift | Several source files still exceed the 1,000-line module target. The current audit found `src/runtime.rs` at 1,192 lines, `src/midge/adapter/documents.rs` at 1,176, `src/midge/adapter/metadata.rs` at 1,143, `src/executor/execution/source.rs` at 1,089, `src/executor/execution/dml.rs` at 1,065, `src/catalog/metadata.rs` at 1,065, and `src/app/query.rs` at 1,064. | [Module Organization targets](module-organization.md), [src/runtime.rs](../src/runtime.rs), [src/midge/adapter/documents.rs](../src/midge/adapter/documents.rs), [src/midge/adapter/metadata.rs](../src/midge/adapter/metadata.rs), [src/executor/execution/source.rs](../src/executor/execution/source.rs), [src/executor/execution/dml.rs](../src/executor/execution/dml.rs), [src/catalog/metadata.rs](../src/catalog/metadata.rs), [src/app/query.rs](../src/app/query.rs) | Large files are the clearest current drift from the small-module architecture and make unrelated query, storage, and runtime changes harder to review. | Owners: runtime, midge adapter, executor, catalog, app. Follow-up: before adding substantial behavior in one of these files, extract a focused module or update [Module Organization](module-organization.md) with a current split plan. |
