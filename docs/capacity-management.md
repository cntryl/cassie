...
Use that report together with host disk measurements for `CASSIE_STORAGE_PATH`, EXPLAIN diagnostics, catalog views, and benchmark scenarios.
...
- deployment-profile notes: record host shape, storage mode, data shape, workload mix, fixture
  scale, `CASSIE_STORAGE_PATH`, runtime threshold overrides, rollback/fallback evidence, and the
