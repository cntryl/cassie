...
Phase 10 is closed for the current whole-system performance rebaseline after Phase 09 production-depth closure.
...
- Issue 06, protocol API startup concurrency and mixed-load efficiency, closed 2026-06-25. Unindexed REST document writes now update row-count cardinality incrementally, REST delete routes through the app delete path, and repeated stable HTTP vector searches reuse versioned query-embedding, normalized-sidecar, and large-result caches without changing REST response shape or async blocking boundaries. `http_document_create_get/100k` improved to p50 `58.339 ms`, p95 `63.281 ms`; `mixed_ingest_query/10k` improved to p50 `29.127 ms`, p95 `31.660 ms`; `http_vector_search/10k` moved from baseline p95 `107.5 ms` and focused-before mean `47.495 ms` to focused diagnostic p50 `8.661 us`, p95 `9.108 us`.
- Issue 07, documentation readiness and capacity reconciliation, closed 2026-06-25. Performance contracts now register the HTTP vector-search scenario used by Issue 06 evidence, production readiness and capacity guidance reference Phase 10 as advisory local evidence, and Phase 00 no longer points at completed Phase 10 issues. No feature family was promoted to production-ready.
