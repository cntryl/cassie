...
pub(super) fn parse_copy_statement(sql: &str) -> Result<ParsedStatement, SqlError> {
    ...
    if !lower.starts_with("copy ") {
        return Err(SqlError::new("COPY requires a target table".into()));
    }
    ...
    let from_pos = lower_after_copy
        .find(" from ")
        .ok_or_else(|| SqlError::new("COPY requires FROM STDIN".into()))?;
    ...
    if !stdin.eq_ignore_ascii_case("stdin") {
        return Err(SqlError::new("COPY only supports FROM STDIN".into()));
    }
...
fn parse_copy_target(raw: &str) -> Result<(String, Vec<String>), SqlError> {
    ...
    let close = raw
        .rfind(')')
        .ok_or_else(|| SqlError::new("COPY column list requires closing ')'".into()))?;
    if close < open || !raw[(close + 1)..].trim().is_empty() {
        return Err(SqlError::new("COPY column list is malformed".into()));
    }
    ...
    if columns_raw.is_empty() {
        return Err(SqlError::new("COPY column list cannot be empty".into()));
    }
...
fn parse_copy_options(
    ...
    if !options.starts_with('(') || !options.ends_with(')') {
        return Err(SqlError::new(
            "COPY WITH options require parentheses".into(),
        ));
    }
        ...
        match parts.as_slice() {
            [key, value] if key.eq_ignore_ascii_case("format") => {
                if !value.eq_ignore_ascii_case("csv") {
                    return Err(SqlError::new("COPY only supports FORMAT csv".into()));
                }
            ...
            _ => {
                return Err(SqlError::new(format!("unsupported COPY option '{option}'")));
            }
        }
    }
...
fn parse_copy_bool(raw: &str) -> Result<bool, SqlError> {
    match raw.to_ascii_lowercase().as_str() {
        ...
        _ => Err(SqlError::new(format!("invalid COPY HEADER value '{raw}'"))),
    }
}
