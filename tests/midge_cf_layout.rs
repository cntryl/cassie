...
use cassie::midge::adapter::{StorageFamily, StorageLayout};
...
fn should_route_schema_data_temp_across_families() {
    ...
    runtime.block_on(async {
        ...
        // Act
        let data_prefix = format!("doc:{collection}:").into_bytes();
        let expected_doc_key = format!("doc:{collection}:{doc_id}").into_bytes();
        ...
        assert!(
            ...
            "document payload should be stored in cf1"
        );
        assert!(
            schema_entries
                .iter()
...
#[test]
fn should_clear_temp_family_during_startup() {
    // Arrange
...
fn should_keep_cassie_metadata_off_default_family() {
    ...
    runtime.block_on(async {
        ...
        for (key, _) in default_entries {
            ...
            assert!(
                !key.starts_with("__cassie__/") && !key.starts_with("doc:"),
