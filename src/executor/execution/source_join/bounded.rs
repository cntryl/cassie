...
pub(super) fn try_execute_streaming_bounded_inner_join(
    ...
    if should_preemptively_dense_stream(env, left_collection, right_collection)? {
        return execute_dense_streaming_bounded_inner_join(
            env,
            left_collection,
            right_collection,
            on,
    ...
    let right_rows = match execute_query_source(env, right, cte_context, true, outer_row, None) {
        Ok((right_batches, _right_text)) => batch::flatten_batches(right_batches),
        Err(error)
...
fn execute_dense_streaming_bounded_inner_join(
    ...
    on: &Expr,
    keys: &EquiJoinKeys,
    left_scan_fields: &[String],
    output_budget: usize,
) -> Result<Vec<BatchRow>, QueryError> {
    let Some(right_scan_fields) = collection_scan_fields(env, right_collection) else {
        return Ok(Vec::new());
    };
    let left_schema = env.cassie.catalog.get_schema(left_collection);
    let right_schema = env.cassie.catalog.get_schema(right_collection);
    let batch_size = env
        .cassie
        .runtime
        .limits()
    ...
    let left_scanned = env.cassie.midge.scan_rows_until::<QueryError, _>(
        ...
        |left_document| {
            ...
            probe_rows += 1;
            let left_key = row_join_key(&left_row, &keys.left);
            ...
            let scanned = env.cassie.midge.scan_rows_until::<QueryError, _>(
                ...
                |right_document| {
                    ...
                    build_rows += 1;
                    if left_key != row_join_key(&right_row, &keys.right) {
                        return Ok(true);
                    }
