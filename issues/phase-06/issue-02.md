# Phase 06 Issue 02: Predicate Order Limit Pushdown
...
- In `src/planner/physical.rs`, add a compact enum such as `ReadAccessPath` or string-backed plan metadata if serialization compatibility is easier:
  ...
  - `ColumnBatchScan`
  - `Fallback`
- Add fields for `access_path`, `access_path_reason`, and optional `ordering_proof` only if issue 06 has not already introduced them.
