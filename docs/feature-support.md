...
| Pgwire results | row description, data row, command complete, ordered result sequences, SQLSTATE-style error response, one final ready for query per simple-query batch | Stable/Experimental | Text results are the baseline; binary codecs and unsupported-format rejection are incomplete by type |
