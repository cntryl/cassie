...
| HNSW vector indexes | Stable persisted graph candidate path with exact source-row reranking. | The functional surface is already promoted. Retain the native-disk recall, latency, resource, fallback, and recovery contract in [HNSW Release Evidence](hnsw-release-evidence.md) without treating Stable as a global capacity claim. |
| IVFFlat vector indexes | Stable persisted membership-prefix candidate path with exact source-row reranking. | The functional surface is already promoted. Retain the bounded list/probe/training recall, latency, resource, fallback, and recovery matrix in [IVFFlat Release Evidence](ivfflat-release-evidence.md) without treating Stable as a global capacity claim. |
